...
class YTMusicSearcher:
    ...
    def search_track(self, track: Dict, max_results: int = 10, verbose: bool = False) -> List[Dict]:
        ...
        if isrc and isrc.strip():
            ...
            try:
                # Search by ISRC - this should give exact match
                isrc_results = self.ytmusic.search(isrc, filter='songs', limit=5)
                ...
                if scored_results:
                    if verbose: print(f"✓ Found exact ISRC match(es): {len(scored_results)}")
        ...
        try:
            # Search for songs only with strict filtering
    ...
    def _process_search_result(self, result: Dict, target_track: Dict, verbose: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            if result.get('isrc_match'):
                ...
                if verbose: print(f"✓ ISRC MATCH CONFIRMED: {isrc}")
            else:
                ...
                if target_isrc and isrc_lower and target_isrc == isrc_lower:
                    ...
                    if verbose: print(f"✓ ISRC MATCH FOUND: {target_isrc}")
